...
### {JT, Octr 2019 / IVA, Sep 2020}

library(here)

###------- Reading outputs of VS model
setwd("~/wrk/VS_intraannual-master") # https://github.com/jantumajer/VS_intraannual
setwd(here())

DailyGr <- read.table("tst rate 1960 - 2012.dat", header = T) # Output of VS on daily scale (rate)
AnnualGr <- read.table("tst 1960 - 2012.dat", header = T, skip = 6) # Output of VS on annual scale
...
for (i in c(1:nrow(DailyGr))){
  ...
  BG <- AnnualGr[AnnualGr$year == year, "SOS"] # Estimated dates of beggining and ending of growing season
  EG <- AnnualGr[AnnualGr$year == year, "EOS"]
  
  if (DailyGr[i, "t"] >= BG & DailyGr[i, "t"] <= EG) {
    DailyGr[i, "CambialActivity"] <- TRUE
  ...
  if (DailyGr[i, "CambialActivity"] == FALSE) {
    DailyGr[i, "Limit"] <- 0
  } else {
...
Formation$BG <- AnnualGr$SOS
Formation$EG <- AnnualGr$EOS
...
### {JT, Oct 2019/ IVA, Sep 2020}


# Reformating data into matrixes
matrix <- matrix(0, ncol=nrow(AnnualGr), 
                 nrow = 366)
...
image(x=as.numeric(colnames(matrix)), y=c(1:nrow(matrix)), z=t(matrix), 
      ylim=c(40, 340), zlim=c(0.001, 1.001),
      xlab = NA, ylab="DOY",
      col = rev(heat.colors(1000)),
      main="Gint"
      ,axes = FALSE)
      
axis(1, at = seq(min(AnnualGr$year), max(AnnualGr$year), by = 5))
axis(2, at = seq(0, 366, by = 30))
axis(4, at = seq(0, 366, by = 30))

# Estimated dates of the start and the end of growing season (BG, EG)
lines(AnnualGr$SOS ~ AnnualGr$year, lw=2, lty=3, col="black")
lines(AnnualGr$EOS ~ AnnualGr$year, lw=2, lty=3, col="black")

# Lineat trends in BG and EG
lines(predict(lm(AnnualGr$SOS ~ AnnualGr$year)) ~ AnnualGr$year, lw=2, lty=3, col="blue")
lines(predict(lm(AnnualGr$EOS ~ AnnualGr$year)) ~ AnnualGr$year, lw=2, lty=3, col="blue")

### Plotting the image of dominant limiting factors

image(x=as.numeric(colnames(matrix.2)), y=c(1:nrow(matrix.2)), z=t(matrix.2), 
      ylim=c(100, 320), col = c("white", "red", "blue", "grey"), axes = F, xlab = NA, ylab="DOY",
      main="dominant limiting factors")
